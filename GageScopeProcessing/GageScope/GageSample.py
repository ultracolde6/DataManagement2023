...
def e3decimate(x, q, n=None, axis=-1, zero_phase=True):
	"""
    Decimates the input signal by applying a Butterworth filter and then downsampling.

    Parameters:
    x (array_like): The input signal.
    q (int): The decimation factor. The number by which the sample rate of the signal is divided.
    n (int, optional): The order of the filter. Defaults to 8.
    axis (int, optional): The axis along which the filter is applied. Defaults to -1 (the last axis).
    zero_phase (bool, optional): If True, a zero-phase digital filter is applied. Defaults to True.

    Returns:
    array_like: The decimated signal.
    """
	q = int(q)
	if n is None:
		n = 8
